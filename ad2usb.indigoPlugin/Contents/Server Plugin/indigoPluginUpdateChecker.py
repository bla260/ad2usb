...
#		## self.updater = indigoPluginUpdateChecker.updateChecker(self, serverUrl [,daysBetweenChecks])
#                     self.updater = indigoPluginUpdateChecker.updateChecker(self [,daysBetweenChecks])
#
#		where:
...
class NO_LONGER_USED_updateChecker(object):
	...
	def __init__(self, plugin, pluginid, daysBetweenChecks=1):
		self.plugin = plugin
		sp = str(pluginid).split('.')
		myName = sp[-1]
		versionServer = 'http://yikes.com/plugins/'
		self.fileUrl = versionServer + myName + '.html'
	...
	def checkVersionNow(self):
		...
		try:
			if (lines[1].startswith('EmailSubject:')) and (lines[2].startswith('EmailBody:')):
				...
				self.plugin.debugLog(u"Emailing the user about the new version.")
