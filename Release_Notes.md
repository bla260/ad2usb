...
- Major changes to Triggers and Events.
  - Triggers created for User Actions have several new features. You can choose if the Event applies to any user or specific user(s). When listing specific users(s) you can continue to provide a single user code number (ex: `02`) or use the new feature of providing a comma seperated list of users (ex: `02,07,11,22`). The valid range for user numbers is 1 to 49. You can also provide an Indigo Variable as the user(s) in the format:`%%v:VARID%%` where `VARID` is the variable ID (ex: `%%v:1929322623%%`). The variable's value can contain one or more user codes.
  - **Panel Arming Events have been deprecated and will be removed in the next release**, use User Actions instead. A User Action with "Any User" is functionally the same as the deprecated Panel Arming Events. To update you Trigger simply change the Event from "Panel Arming Events (deprecated)" to "User Actions." The type of Alarm Event and Partition will be set based on the value of your Panel Arming Event setting and the new field "Any User" will be selected by default. Upon saving the Trigger it will be migrated. With this release any Triggers you have based on Panel Arming Events will not allow you to changes the Panel Arming Events details but they will continue to work. The Event Log will continue to warn you these type of Trigger Events exist on startup or execution of the Trigger. While you can rollback this release of the plugin, Panel Arming Events Triggers that were migrated will remain User Action Triggers after the rollback and you will need to manually change these back to Panel Arming Events.
...
- Added all the remaining Long Range Radio (LRR) events for processing triggers with the newer AlarmDecoder firmware version 2.2a.8.8 except for "Alarm Aux" which has not easily matched SAIC code. The README indicates which of these events have been tested on an actual panel and which have not.
