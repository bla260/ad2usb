v 3.1.0 July 22, 2022
- **IMPORTANT:** Python 3 version. Requires Indigo 2022.1 or later. Read the v 3.0.0 release notes below first if you're upgrading from 1.x.
- **IMPORTANT:**  Changes to Alarm Keypad (ad2usb Keypad) device panel Ready/Armed states. Internal to the plugin, the device state `displayState` for keypad devices has been removed. This means if you had any Indigo Device Triggers based on the AD2SUB Keypad Device state (e.g. `Alarm state changed to...` -  usually the first options in the Trigger pull down) you should disable these Triggers before upgrading and then edit and re-save each of these Triggers after upgrading. Other Triggers and states associated with the ad2usb Keypad are not impacted by this change; only the Alarm Armed State. The list of valid states are: Ready (Not Armed), Fault, Armed Stay, Armed Away, and Error. Additional states Armed Night-Stay, Armed Instant, and Armed Max will be added in a future release.
